 ...
 * 单位为毫秒，默认值2000毫秒；
 * 当屏幕刷新间隔超过阈值，则上报QYMoviePlayerRefreshTimeoutNotification通知。通知使用@"timeout"键值对来提供具体时长数据，单位为毫秒。每增加一秒提示一次。
 * 阈值只能在 [prepareToPlay]([QYMediaPlayback prepareToPlay]) 前设置，后续设置无法生效。
 
 使用以下代码获取超时时长，超时时长单位毫秒：
 <pre><code>
[[[notification userInfo] valueForKey:@"timeout"] intValue]
 </code></pre>
