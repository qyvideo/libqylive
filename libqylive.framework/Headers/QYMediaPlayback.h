 ...
 * 如果isPreparedToPlay为FALSE，直接调用[play]([QYMediaPlayback play])，则在play内部自动调用[prepareToPlay]([QYMediaPlayback prepareToPlay])接口。
 * prepareToPlay调用后，由MPMediaPlaybackIsPreparedToPlayDidChangeNotification通知完成准备工作。查询[isPreparedToPlay]([QYMediaPlayback isPreparedToPlay])可以获得具体属性值。
 @discussion 通知类型
 ...
 * 如果isPreparedToPlay为TRUE，则可以调用[play]([QYMediaPlayback play])接口开始播放;
 * 如果isPreparedToPlay为FALSE，则需要调用[prepareToPlay]([QYMediaPlayback prepareToPlay])接口开始准备工作；
 * 如果isPreparedToPlay为FALSE，直接调用[play]([QYMediaPlayback play])，则在play内部自动调用[prepareToPlay]([QYMediaPlayback prepareToPlay])接口。
 @see prepareToPlay
 */
 ...
 * 如果调用play方法前已经调用[prepareToPlay]([QYMediaPlayback prepareToPlay])完成播放器对视频文件的初始化，且[shouldAutoplay]([QYMoviePlayerController shouldAutoplay])属性为NO，则调用play方法将开始播放当前视频。此时播放器状态为CBPMoviePlaybackStatePlaying。
 * 如果调用play方法前已经调用[prepareToPlay]([QYMediaPlayback prepareToPlay])完成播放器对视频文件的初始化，且[shouldAutoplay]([QYMoviePlayerController shouldAutoplay])属性为YES，则调用play方法将暂停播放当前视频，实现效果和pause一致。
 * 如果调用play方法前未调用[prepareToPlay]([QYMediaPlayback prepareToPlay])完成播放器对视频文件的初始化，则播放器自动调用[prepareToPlay]([QYMediaPlayback prepareToPlay])进行视频文件的初始化工作。
 * 如果调用play方法前已经调用pause暂停了正在播放的视频，则重新开始启动播放视频。
 @since Available in QYMediaPlayback 1.0 and later.
 @see prepareToPlay
 */
 ...
 * 重新回到播放状态，需要调用[play]([QYMediaPlayback play])方法。
 ...
 * 调用stop结束当前播放，如果需要重新播放该视频，需要调用[prepareToPlay]([QYMediaPlayback prepareToPlay])方法。
